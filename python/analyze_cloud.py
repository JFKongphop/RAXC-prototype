"""
RAXC Analyzer (Cloud) — RAG-powered smart contract vulnerability scanner using Qdrant Cloud.
Usage:
  python3 analyze_cloud.py                        # uses built-in sample contract
  python3 analyze_cloud.py path/to/contract.sol   # analyze a specific file
...
def analyze(contract_code: str) -> tuple:
    print(f"[*] Querying Qdrant Cloud: {os.getenv('QDRANT_URL', 'localhost:6333')}")
...
- You MUST write ALL cases completely. Do NOT summarize, skip, or abbreviate any case. Do NOT end with any generic closing note such as "apply similar changes elsewhere", "similar changes should be applied to other functions", "repeat this pattern for other functions", or any equivalent phrase — these are FORBIDDEN. Every single case must be written in complete, standalone, runnable Solidity code.
"""
...
def save_markdown(report: str, results: list, contract_name: str = "contract", func_matches: list = None) -> str:
    ...
    filepath = out_dir / f"RAXC_cloud_{contract_name}_{timestamp}.md"
    ...
    # ── Header ──────────────────────────────────────────────────────────────
    lines.append("# RAXC Security Report (Cloud)")
    ...
    lines.append("> *Embeddings: OpenAI text-embedding-3-small · LLM: GPT-4o · Vector DB: Qdrant Cloud*")
...
if __name__ == "__main__":
  if len(sys.argv) > 1 and sys.argv[1] == "api":
    ...
    async def analyze_contract(contract: UploadFile = File(...)):
      try:
        code = await contract.read()
        code = code.decode("utf-8")
      except Exception:
        raise HTTPException(status_code=400, detail="Invalid file encoding")

      report, results = analyze(code)
      func_matches = match_functions(code)
      ...
      return {"download_link": f"/reports/{filename}"}
    ...
    async def get_report(filename: str):
      ...
      return FileResponse(str(report_path), media_type="text/markdown", filename=filename)
