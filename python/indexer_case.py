"""
RAXC Indexer — Index DeFiVulnLabs vulnerability pattern .sol files into Qdrant.
Source: datasets-case-exploit/src/test/*.sol
Target collection: defi_cases  (educational patterns — separate from real exploit POCs)
Run: python3 indexer_case.py
"""
...
# Skip helper/interface files that aren't vulnerability patterns
SKIP_FILES = {"interface.sol"}


# ---------------------------------------------------------------------------
# Collection setup (create only if missing — won't overwrite existing data)
# ---------------------------------------------------------------------------
...
def vuln_type_from_name(stem: str) -> str:
    """Convert filename stem to a cleaned vuln_type label."""
...
def parse_sol_file(filepath: str) -> dict:
    ...
    path    = Path(filepath)
    stem    = path.stem                          # e.g. "Reentrancy"
    vuln    = vuln_type_from_name(stem)          # e.g. "reentrancy"
